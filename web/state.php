...
$state_location = "/home/pi/moto_x/arsed_state.json";
...
if ($_GET['u']) {
	...
	$j['radios'] = "$json Radios: $state[ars_clients_here] registered on network, $state[ars_clients] total";
	...
	foreach ($here as $radio) {
		...
		$s .= "<tr class='here'>
			...
			<td>" . $radio['name'] . "</td>
			<td><a href='https://database.radioid.net/database/view?id=$radio[id]#!' target='blank'>" . $radio['id'] . "</a></td>
			<td>" . (isset($radio['heard_what']) ? $radio['heard_what'] : '') . "</td>
			<td>" . $radio['latitude'] . "</td>
			<td>" . $radio['longitude'] . "</td>
			<td>" . logtime($radio['first_heard']) . "</td>
